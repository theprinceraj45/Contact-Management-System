...
def OnSelected(event):
    ...
    #LABELS
    lbl_title = Label(FormTitle, text="Updating Contacts", font=('arial', 16), bg="pink",  width = 300)
...
def AddNewWindow():
    ...
    CONTACT.set("")
    NewWindow = Toplevel()
    NewWindow.title("Prince's Contact")
    width = 400
    height = 300
    bg="red"
    screen_width = root.winfo_screenwidth()
    ...
    #LABELS
    lbl_title = Label(FormTitle, text="Adding New Contacts", font=('arial', 16), bg="#32cd32",  width = 300)
    lbl_title.pack(fill=X)
    lbl_firstname = Label(ContactForm, text="Firstname", font=('arial', 14), bd=5)
    lbl_firstname.grid(row=0, sticky=W)
    lbl_lastname = Label(ContactForm, text="Lastname", font=('arial', 14), bd=5)
...
Top.pack(side=TOP)
Mid = Frame(root, width=500,  bg="#87ceeb")
Mid.pack(side=TOP)
...
#=LABELS
lbl_title = Label(Top, text="PEC Students Contact Management System", font=('arial', 16), width=500, bg="#ff6347")
lbl_title.pack(fill=X)

#ENTRY

#BUTTONS
btn_add = Button(MidLeft, text="+ ADD NEW CONTACT", bg="#ffa500", command=AddNewWindow)
btn_add.pack()
btn_delete = Button(MidRight, text="DELETE RECORD", bg="#66ff66", command=DeleteData)
